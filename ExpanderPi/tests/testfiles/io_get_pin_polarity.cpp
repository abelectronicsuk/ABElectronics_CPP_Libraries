/*
 * io_get_pin_polarity.cpp
 * 
 * ExpanderPi class > io_get_pin_polarity function test program
 *   
*/
...
{
	TestLibs::start_test("ExpanderPi class > io_get_pin_polarity()");
	...
	for (uint8_t a = 1; a < 17; a++){
		...
		TestLibs::i2c_emulator_write_word_data(TestLibs::MCP23017_IPOLA, y);
        x = bus.io_get_pin_polarity(a);
        if (x != 0){
