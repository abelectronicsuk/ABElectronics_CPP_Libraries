/*
 * io_get_pin_pullup.cpp
 * 
 * ExpanderPi class > io_get_pin_pullup function test program
 *   
*/
...
{
	TestLibs::start_test("ExpanderPi class > io_get_pin_pullup()");
	...
	for (uint8_t a = 1; a < 17; a++){
		...
		TestLibs::i2c_emulator_write_word_data(TestLibs::MCP23017_GPPUA, y);
        x = bus.io_get_pin_pullup(a);
        if (x != 0){
